...
numOfRecords = input("Please enter how many records you have: ")
...
for x in range(int(numOfRecords)):
    print("Please enter data for entry " + str(x+1))
    studentInput = input(":___,".join(columns)+":___\n")  # Print out the format of how input should be
    studentInput = studentInput.split(",")  # After getting input, split into n objects based on the "," comma

    while len(studentInput) != len(columns):  # Data verification, make sure they entired the right amt of items
